...
exports.getMyTour = catchAsync(async (req, res, next) => {
  ...
  const tourIds = bookings.map(el => el.tour._id); //it will look all bookings and create an array of booking Ids
  const tours  = await Tour.find({ _id : { $in : tourIds } }); //find all tours which have ids in tourIds array
  res.status(200).render('overview', {
    title: 'My Tours',
    tours
  });
